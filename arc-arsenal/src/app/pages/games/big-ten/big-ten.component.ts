...
@Component({
  ...
  templateUrl: './big-ten.component.html',
  styleUrl: './big-ten.component.scss',
})
export class BigTenGameComponent implements OnDestroy{
  ...
  readonly localStorageItemName = 'bigTenGame';
  ...
  calculateScoreSum(scores: (number | 'X' | 'M')[]): number {
    return scores.reduce((total: number, s) => {
      if (s === 'X' || s === 10 || s === 9) return total + 10;
      if (s === 'M') return total;
